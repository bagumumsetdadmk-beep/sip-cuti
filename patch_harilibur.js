...
const importMatch = `import { Calendar, Plus, Edit2, Trash2, X } from 'lucide-react';`;
const newImport = `import { Calendar, Plus, Edit2, Trash2, X, Search } from 'lucide-react';
...
const filterMatch = `  const sortedLibur = [...hariLibur].sort((a, b) => new Date(a.tanggal).getTime() - new Date(b.tanggal).getTime());`;
const newFilterMatch = `  const sortedLibur = [...hariLibur].sort((a, b) => new Date(a.tanggal).getTime() - new Date(b.tanggal).getTime());
  
  const filteredLibur = sortedLibur.filter(l => l.keterangan.toLowerCase().includes(searchTerm.toLowerCase()));
...
const mapMatch = `              {sortedLibur.length === 0 ? (
                <tr>
                ...
                sortedLibur.map((l, idx) => (`;
const newMapMatch = `              {filteredLibur.length === 0 ? (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-gray-400 text-sm">
                    Belum ada data hari libur.
                  </td>
...
console.log('Success patch_harilibur.js');
