...
const importMatch = `import { Network, Plus, Edit2, Trash2, X } from 'lucide-react';`;
const newImport = `import { Network, Plus, Edit2, Trash2, X, Search } from 'lucide-react';
...
const filterMatch = `  const sortedAtasan = [...atasan].sort((a, b) => a.jabatan.localeCompare(b.jabatan));`;
const newFilterMatch = `  const sortedAtasan = [...atasan].sort((a, b) => a.jabatan.localeCompare(b.jabatan));
  
  const filteredAtasan = sortedAtasan.filter(a => {
    const searchLower = searchTerm.toLowerCase();
    const p = pegawai.find(peg => peg.id === a.pegawaiId);
    return a.jabatan.toLowerCase().includes(searchLower) || (p && p.nama.toLowerCase().includes(searchLower));
  });
  const totalPages = Math.ceil(filteredAtasan.length / itemsPerPage);
  const paginatedAtasan = filteredAtasan.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);`;
...
const mapMatch = `              {sortedAtasan.length === 0 ? (
                <tr>
                  <td colSpan={3} className="p-8 text-center text-gray-400 text-sm">
                    Belum ada data atasan/pejabat.
                  </td>
                </tr>
              ) : (
                sortedAtasan.map((a, idx) => {
                  const p = pegawai.find(peg => peg.id === a.pegawaiId);
                  return (`;
const newMapMatch = `              {filteredAtasan.length === 0 ? (
                <tr>
                  <td colSpan={3} className="p-8 text-center text-gray-400 text-sm">
                    Belum ada data atasan/pejabat.
                  </td>
...
const newTableMatch = `          </table>
        ...
        <Pagination
          ...
          totalItems={filteredAtasan.length}
...
console.log('Success patch_atasan.js');
